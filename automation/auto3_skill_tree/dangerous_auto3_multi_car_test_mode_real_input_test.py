...
class Auto3MultiCarTestModeRealInputError(Exception):
    """Raised when guarded Auto3 multi-car real-input execution cannot start."""


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Dangerous manual Auto3 multi-car test-mode real-input test. "
            "No perk unlock actions are included. Example: "
            "python -B -m automation.auto3_skill_tree."
            "dangerous_auto3_multi_car_test_mode_real_input_test "
            "--cars 2 --confirm-real-input"
        )
    )
    parser.add_argument(
        "--cars",
        type=int,
        required=True,
        help=f"Required. Number of cars to test. Must be 1-{MAX_CARS}.",
    )
    parser.add_argument(
        "--confirm-real-input",
        action="store_true",
    ...
    command_label = "Auto3 multi-car test-mode real-input test"
    ...
    try:
        profile_data = load_multi_car_test_mode_profile(args.fast, args.profile)
    except ProfileSelectionError as error:
        logger.error(
            "Auto3 multi-car test-mode profile selection failed: %s",
            error,
            category="profile",
        )
        print_error(f"Auto3 multi-car test-mode profile selection failed: {error}")
        return 1

    print_command_intro(
        "Auto3 Multi-Car Test-Mode Real-Input Validation",
    ...
    logger.warning(
        "Auto3 multi-car test-mode real-input starting.",
        category="sequence",
    )
    ...
    except Auto3MultiCarTestModeRealInputError as error:
        logger.error(
            "Auto3 multi-car test-mode real-input unavailable: %s",
            error,
            category="error",
        )
        print_error(f"Auto3 multi-car test-mode real-input unavailable: {error}")
        return 1

    print_run_summary(
        RunSummary(
            status=CommandStatus(result.status),
            command="Auto3 Multi-Car Test-Mode Real-Input Validation",
            fields=[
                ("Mode", "multi-car-test"),
                ("Cars", args.cars),
                ("Profile", profile_data["profile_id"]),
            ...
            operator_message=OperatorMessage(
                message="Auto3 test-mode traversal returned control.",
                suggested_next_step=(
                    "Verify the grid baseline before any unlock validation."
                ),
    ...
    if result.completed:
        logger.warning(
            "Auto3 multi-car test-mode real-input completed.",
            category="sequence",
        )
        return 0
    ...
    if result.stopped:
        logger.warning(
            "Auto3 multi-car test-mode real-input stopped.",
            category="sequence",
        )
        return 1

    logger.error(
        "Auto3 multi-car test-mode real-input failed: %s",
        result.message,
        category="error",
    )
...
def build_car_count_refusal(message: str, car_count: int) -> CommandResult:
    return CommandResult(
        ...
        operator_message=OperatorMessage(
            ...
            suggested_next_step=(
                "Use the validated A-start traversal boundary before expanding "
                "test coverage."
            ),
            risk_level=RiskLevel.LOW,
        ),
...
def run_auto3_multi_car_test_mode_real_input(
    car_count: int,
    profile_data: dict,
    ...
    try:
        ...
        raise Auto3MultiCarTestModeRealInputError(str(error)) from error

    runner = Auto3SkillTreeRunner(
        input_controller=input_controller,
        stop_manager=stop_manager,
        profile_data=profile_data,
        action_builder=lambda profile: build_auto3_multi_car_test_actions(
            car_count,
            profile,
        ),
        logger=logger,
...
def load_multi_car_test_mode_profile(
